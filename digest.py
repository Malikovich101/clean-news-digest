...
AI_BATCH_SIZE = 30
STATE_VERSION = 1
AD_MARKERS = ("#реклама", "erid", "на правах рекламы", "рекламная интеграция", "партнёрский материал", "партнерский материал")
PROMO_PATTERNS = (r"\bпромокод\b", r"\bскидк\w*\b", r"\bкуп\w*\b", r"\bзаказ\w*\b", r"\bрегистр\w*\b", r"\bрозыгрыш\w*\b")
...
def fetch_posts(client: TelegramClient, channels: list[str], state: dict[str, Any], since: datetime) -> list[Post]:
    ...
    for channel in channels:
        ...
        for message in client.iter_messages(entity, min_id=watermark):
            ...
            if text.strip():
                channel_posts.append(Post(f"{channel}:{message.id}", channel, date.isoformat(), text, source_url(channel, int(message.id))))
        posts.extend(channel_posts)
        state.setdefault("watermarks", {})[channel] = max_seen_id
        print(f"[telegram] {channel}: {len(channel_posts)} text posts", flush=True)
    return posts
